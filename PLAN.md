...
- 表单必填：宠物名、学员昵称、展示图、宠物 ZIP 包、公开展示确认。
- GitHub 账号直接读取 Issue 作者。
- 展示图接受 PNG、GIF、WebP；ZIP 只作为下载附件，不解析、不运行。
...
- 点击卡片打开详情层，提供大图、GitHub 主页、原 Issue 和 ZIP 下载入口。
...
- 使用真实 Issue Form 提交一张测试图片和一个测试 ZIP。
- 确认工作流成功，投稿在 1–3 分钟内出现在 Pages。
- 确认卡片内容、详情层、作者链接、Issue 链接和 ZIP 下载均正确。
