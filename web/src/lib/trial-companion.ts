...
export const TRIAL_BOTTOM_MARGIN = 28;
...
export function clampTrialPosition(
  ...
  const maxY = Math.max(TRIAL_EDGE_MARGIN, viewport.height - size.height - TRIAL_BOTTOM_MARGIN);
  return {
    x: Math.min(maxX, Math.max(TRIAL_EDGE_MARGIN, position.x)),
    y: Math.min(maxY, Math.max(TRIAL_EDGE_MARGIN, position.y)),
  };
}
